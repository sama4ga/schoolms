...
if (!$con->query("DESCRIBE `fees`")) {
  echo "Setup fees table not created";
  exit();
}


// create fees table
$fees_id="fees_".$session."_".$term;
...
$con->query( "CREATE TABLE IF NOT EXISTS `fees_debtors`(
              `fees_id` INT(10) NOT NULL AUTO_INCREMENT, PRIMARY KEY (`fees_id`),
              `std_id` INT(10) NOT NULL ,
              `surname` varchar(250) NOT NULL,
              `othernames` varchar(400) NOT NULL,
  ...
  if ($result) {
    ...
    echo "Error getting student data ".$con->error;
    exit();
  }
...
if ($result ) {
  ...
  //echo $jss_1_fees." ".$jss_2_fees." ".$jss_3_fees." ".$ss_1_fees." ".$ss_2_fees." ".$ss_3_fees;
    ...
    echo "No fees has been setup for the selected session/term ($session_formatted/$term)";
    exit();
  }
...
// populate fees debtors table with student data
  ...
  $sql="INSERT INTO `fees_debtors`(
                  `std_id`,`surname`,`othernames`,`class`,`arm`,`term`,`session`,`fees`,`amount_paid`,`balance`
                  )VALUES";
  for ($i=0; $i < $num_students; $i++) { 
    ...
    if ($i == $num_students-1) {
      ...
      $sql.="('$std_id[$i]','$surname[$i]','$othernames[$i]','$class[$i]','$arm[$i]','$term','$session_formatted','$fees','0','$fees');";
